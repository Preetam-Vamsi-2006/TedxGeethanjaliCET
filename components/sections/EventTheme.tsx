"use client";

import { useRef } from "react";
import { motion } from "framer-motion";
import { useInView } from "react-intersection-observer";
import { EVENT_THEME } from "@/lib/data";

export default function EventTheme() {
  const { ref, inView } = useInView({ threshold: 0.1, triggerOnce: true });

  return (
    <section
      id="theme"
      ref={ref}
      className="relative py-20 sm:py-32 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-black to-black overflow-hidden"
    >
      {/* Animated background */}
      <div className="absolute inset-0 -z-10">
        <motion.div
          className="absolute top-0 right-0 w-96 h-96 bg-ted rounded-full mix-blend-multiply filter blur-3xl opacity-20"
          animate={{ y: [0, 50, 0], x: [0, 30, 0] }}
          transition={{ duration: 8, repeat: Infinity }}
        />
      </div>

      <div className="max-w-5xl mx-auto">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={inView ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }}
          transition={{ duration: 0.6 }}
          className="text-center mb-16"
        >
          <h2 className="text-4xl sm:text-5xl font-bold mb-4 text-white">Our Theme</h2>
          <div className="w-16 h-1 bg-ted mx-auto" />
        </motion.div>

        {/* Theme description card */}
        <motion.div
          initial={{ opacity: 0, y: 40 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true, amount: 0.3 }}
          transition={{ duration: 0.7, ease: "easeOut" }}
          className="group mb-12"
        >
          <motion.div
            className="p-8 rounded-2xl h-full backdrop-blur-md bg-black border border-ted/50 hover:border-ted transition-all duration-500"
            whileHover={{ 
              y: -8,
              boxShadow: "0 20px 50px rgba(230, 57, 70, 0.15)"
            }}
          >
            {/* Title with smooth reveal */}
            <motion.h3 
              className="text-2xl font-bold mb-4 text-ted"
              initial={{ opacity: 0 }}
              whileInView={{ opacity: 1 }}
              transition={{ duration: 0.5, delay: 0.1 }}
            >
              {EVENT_THEME.title}
            </motion.h3>

            {/* Animated divider */}
            <motion.div
              className="h-0.5 w-12 bg-gradient-to-r from-ted to-ted-light mb-4 rounded"
              initial={{ width: 0 }}
              whileInView={{ width: 48 }}
              transition={{ duration: 0.6, delay: 0.2 }}
            />

            {/* Description with fade effect */}
            <motion.p 
              className="text-white leading-relaxed"
              initial={{ opacity: 0 }}
              whileInView={{ opacity: 1 }}
              transition={{ duration: 0.6, delay: 0.3 }}
            >
              {EVENT_THEME.description}
            </motion.p>

            {/* Bottom hover line */}
            <motion.div
              className="mt-6 h-0.5 bg-gradient-to-r from-ted via-ted-light to-transparent rounded"
              initial={{ width: 0 }}
              whileHover={{ width: "100%" }}
              transition={{ duration: 0.4 }}
            />
          </motion.div>
        </motion.div>

        {/* Key points - same card style as About section */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {EVENT_THEME.keyPoints.map((point, idx) => (
            <motion.div
              key={idx}
              initial={{ opacity: 0, y: 40 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true, amount: 0.3 }}
              transition={{ duration: 0.7, delay: idx * 0.1, ease: "easeOut" }}
              className="group"
            >
              <motion.div
                className="p-4 rounded-xl backdrop-blur-md bg-black border border-ted/50 hover:border-ted transition-all duration-500"
                whileHover={{ 
                  y: -4,
                  boxShadow: "0 20px 50px rgba(230, 57, 70, 0.15)"
                }}
              >
                {/* Animated divider */}
                <motion.div
                  className="h-0.5 w-8 bg-gradient-to-r from-ted to-ted-light mb-3 rounded"
                  initial={{ width: 0 }}
                  whileInView={{ width: 32 }}
                  transition={{ duration: 0.6, delay: idx * 0.1 + 0.2 }}
                />

                {/* Content */}
                <motion.p 
                  className="text-white text-sm font-medium leading-tight"
                  initial={{ opacity: 0 }}
                  whileInView={{ opacity: 1 }}
                  transition={{ duration: 0.6, delay: idx * 0.1 + 0.3 }}
                >
                  {point}
                </motion.p>
              </motion.div>
            </motion.div>
          ))}
        </div>
      </div>
    </section>
  );
}
